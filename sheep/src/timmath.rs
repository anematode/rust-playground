...
fn steps_until_all_equal(value_count: usize, verbose: bool) -> usize {
...
fn get_avg_equilibrium_steps(value_count: usize, trials: usize) -> f64 {
    let total_steps = (0..trials)
        .map(|_| steps_until_all_equal(value_count, trials == 1))
        .sum::<usize>() as f64;
...
pub fn main() {
    loop {
        ...
        match (
            ...
            (Err(err), _) => println!(
                "{} Failed to parse number of values: {}",
                "[your error]".red(),
                err
            ),
            (_, Err(err)) => println!(
                "{} Failed to parse number of trials: {}",
                "[your error]".red(),
                err
            ),
        }
